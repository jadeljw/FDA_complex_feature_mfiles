...
decoding_acc_mean = zeros(1,length(timelag));
...
for r = 11 : 22
    ...
    %     p =strcat( 'E:\DataProcessing\FDA_complex_feature\decoding_result\CCA_SL_sound_EEG\train_set\',band_name(2:end));
    ...
    for  j = 1 : length(timelag)
        ...
        decoding_acc_mean(j)= mean(mean(decoding_correct_or_not_mean,2));
        
    end
    
    figure; plot((1000/Fs)*timelag,decoding_acc_mean*100,'k');
    hold on;plot((1000/Fs)*timelag(Decoding_acc_ttest_result>0),decoding_acc_mean(Decoding_acc_ttest_result>0)*100,'r*');
    xlabel('Times(ms)'); ylabel('Decoding accuracy(%)')
    saveName3 = strcat('Decoding-Acc across all time-lags train set using CCA-SL method',band_name,'.jpg');
    title(saveName3(1:end-4));
    legend('feature:CCA speaker-listener','significant ¡Ù 50%');ylim([60,100]);
