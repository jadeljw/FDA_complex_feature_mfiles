...
%     listener_time_index =  (start_time+5)*Fs+1:(end_time+5)*Fs; % 10 s - 35s
    ...
    listener_time_index =  (start_time+0)*Fs+1:(end_time+0)*Fs;
...
%         load('E:\DataProcessing\afterICA_data\ISC_all_ref_after_ICA_64Hz_0.1-40Hz.mat')
    ...
    %% Channel Index
%     chn_sel_index= 1:60;
    chn_sel_index = [6:14 16:22 25:31 34:40 43:49];
...
%     timelag = 0;
    ...
    for j =  1 : length(timelag)
        for listener = 1 : 12
            ...
            disp('combining data ...');
            tic;
            eval(strcat('eeg_dual_',dataName,'_total=zeros(length(chn_sel_index),15*length(listener_time_index));'));
        ...
        %% correlation
        recon_AttendDecoder_audioA_cca = zeros(12,15);
        recon_UnattendDecoder_audioB_cca = zeros(12,15);
        recon_AttendDecoder_audioB_cca = zeros(12,15);
        recon_UnattendDecoder_audioA_cca = zeros(12,15);
        
        p_recon_AttendDecoder_audioA_cca = zeros(12,15);
        p_recon_UnattendDecoder_audioB_cca = zeros(12,15);
        p_recon_AttendDecoder_audioB_cca = zeros(12,15);
        p_recon_UnattendDecoder_audioA_cca = zeros(12,15);
        ...
        % train matrix
        recon_AttendDecoder_audioA_train_cca = cell(12,15); % training story numbers, listener, story
        recon_UnattendDecoder_audioB_train_cca = cell(12,15);
        recon_AttendDecoder_audioB_train_cca = cell(12,15);
        recon_UnattendDecoder_audioA_train_cca = cell(12,15);
        
        p_recon_AttendDecoder_audioA_train_cca = cell(12,15);
        p_recon_UnattendDecoder_audioB_train_cca = cell(12,15);
        p_recon_AttendDecoder_audioB_train_cca = cell(12,15);
        p_recon_UnattendDecoder_audioA_train_cca = cell(12,15);
        
        train_data_labels = cell(12,15);
        
        for listener = 1 : 12
            
            train_cca_attend_mean = zeros(length(chn_sel_index),15);
            train_cca_attend_r = zeros(length(chn_sel_index),15);
            train_cca_unattend_mean = zeros(length(chn_sel_index),15);
            train_cca_unattend_r = zeros(length(chn_sel_index),15);
            ...
            for story = 1 : 15
                ...
                %zscore              
                [~, story_train_EEG_invalid_row] = find(abs(story_train_EEG)>zscore_range);
                over_range_index = unique(story_train_EEG_invalid_row);
                ...
                if ~isempty(over_range_index)
                    ...
                    story_train_EEG(:,unique(story_train_EEG_invalid_row))=[];
                    story_train_audio_Attend(:,unique(story_train_EEG_invalid_row))=[];
                    story_train_audio_unAttend(:,unique(story_train_EEG_invalid_row))=[];
                end
                ...
                for train_story = 1 : 15
                    ...
                    if train_story ~= story
                        ...
                        [recon_AttendDecoder_audioA_train_cca{listener,story}(cnt_train_story),p_recon_AttendDecoder_audioA_train_cca{listener,story}(cnt_train_story)] =...
                            corr(reconstruction_audio_attend',story_train_audio_A');
                        [recon_AttendDecoder_audioB_train_cca{listener,story}(cnt_train_story),p_recon_AttendDecoder_audioB_train_cca{listener,story}(cnt_train_story)] = ...
                            corr(reconstruction_audio_attend',story_train_audio_B');
                        
                        [recon_UnattendDecoder_audioB_train_cca{listener,story}(cnt_train_story),p_recon_UnattendDecoder_audioB_train_cca{listener,story}(cnt_train_story)] = ...
                            corr(reconstruction_audio_unattend',story_train_audio_B');
                        
                        [recon_UnattendDecoder_audioA_train_cca{listener,story}(cnt_train_story),p_recon_UnattendDecoder_audioA_train_cca{listener,story}(cnt_train_story)] = ...
                ...
                [recon_AttendDecoder_audioA_cca(listener,story),p_recon_AttendDecoder_audioA_cca(listener,story)] =...
                    corr(reconstruction_audio_attend',story_predict_audio_A');
                [recon_AttendDecoder_audioB_cca(listener,story),p_recon_AttendDecoder_audioB_cca(listener,story)] = ...
                    corr(reconstruction_audio_attend',story_predict_audio_B');
                
                [recon_UnattendDecoder_audioB_cca(listener,story),p_recon_UnattendDecoder_audioB_cca(listener,story)] = ...
                    corr(reconstruction_audio_unattend',story_predict_audio_B');
                
                [recon_UnattendDecoder_audioA_cca(listener,story),p_recon_UnattendDecoder_audioA_cca(listener,story)] = ...
        ...
        %             'recon_AttendDecoder_attend_train_cca','recon_UnattendDecoder_unattend_train_cca' ,'recon_AttendDecoder_unattend_train_cca','recon_UnattendDecoder_attend_train_cca',...
        %             'p_recon_AttendDecoder_attend_train_cca','p_recon_UnattendDecoder_unattend_train_cca', 'p_recon_AttendDecoder_unattend_train_cca','p_recon_UnattendDecoder_attend_train_cca');
        
        
        saveName = strcat('cca_sound_EEG_result+',num2str((1000/Fs)*timelag(j)),'ms',band_name{bandName},'.mat');
        save(saveName,'recon_AttendDecoder_audioA_cca','recon_UnattendDecoder_audioA_cca' ,'recon_AttendDecoder_audioB_cca','recon_UnattendDecoder_audioB_cca',...
            'p_recon_AttendDecoder_audioA_cca','p_recon_UnattendDecoder_audioA_cca', 'p_recon_AttendDecoder_audioB_cca','p_recon_UnattendDecoder_audioB_cca',...
            'recon_AttendDecoder_audioA_train_cca','recon_UnattendDecoder_audioA_train_cca' ,'recon_AttendDecoder_audioB_train_cca','recon_UnattendDecoder_audioB_train_cca',...
            'p_recon_AttendDecoder_audioA_train_cca','p_recon_UnattendDecoder_audioA_train_cca', 'p_recon_AttendDecoder_audioB_train_cca','p_recon_UnattendDecoder_audioB_train_cca',...
            'train_data_labels');
        
        %         %plot
        %         % reconstruction accuracy plot attend
        %         figure; plot(mean(recon_AttendDecoder_audioA_cca,2),'r');
        %         hold on; plot(mean(recon_AttendDecoder_audioB_cca,2),'b');
        ...
        %         figure; plot(mean(recon_UnattendDecoder_audioA_cca,2),'r');
        %         hold on; plot(mean(recon_UnattendDecoder_audioB_cca,2),'b');
        %         xlabel('Subject No.'); ylabel('r value')
        ...
        %         Decoding_result_attend_decoder = recon_AttendDecoder_audioA_cca-recon_AttendDecoder_audioB_cca;
        %         Individual_subjects_result_attend = sum(Decoding_result_attend_decoder>0,2)/15;
        %         mean(Individual_subjects_result_attend)
        %         Decoding_result_unattend_decoder = recon_UnattendDecoder_audioB_cca-recon_UnattendDecoder_audioA_cca;
        ...
        %         recon_AttendDecoder_attend_total(j) =  mean(mean(recon_AttendDecoder_audioA_cca));
        %         recon_AttendDecoder_unattend_total(j) =  mean(mean(recon_AttendDecoder_audioB_cca));
        %         recon_UnattendDecoder_attend_total(j) = mean(mean(recon_UnattendDecoder_audioA_cca));
        %         recon_UnattendDecoder_unattend_total(j) = mean(mean(recon_UnattendDecoder_audioB_cca));
        %
        %         %decoding accuracy
        %         Decoding_result_attend_decoder = recon_AttendDecoder_audioA_cca-recon_AttendDecoder_audioB_cca;
        %         Individual_subjects_result_attend = sum(Decoding_result_attend_decoder>0,2)/15;
